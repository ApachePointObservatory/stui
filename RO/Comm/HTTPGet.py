...
class HTTPGet(RO.AddCallback.BaseMixin):
	...
	def _httpDoneCallback(self, token=None):
		...
		self.setState(self.Done, errMsg)
	...
	def setState(self, newState, errMsg=None):
		...
		"""
		if _Debug:
			print "%s.setState(newState=%s, errmsg=%r)" % (self, newState, errMsg)
	...
	def __cleanup(self):
		...
		Private: call only from setState!
