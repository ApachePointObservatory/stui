...
"""Take a series of DIS exposures at different focus positions to estimate best focus.

This script imports the standard DIS exposure widget
...
History:
2006-11-02 ROwen	Start from NICFPS:Focus script.
"""
import math
import numarray
import Numeric
...
DefBoresightYPos = -20 # boresight Y position (arcsec) (X is 0)
...
PlotTitle = "Private.DIS Focus Plot"
...
class ScriptClass(object):
	def __init__(self, sr):
		...
		self.guideModel = TUI.Guide.GuideModel.getModel("dcam")
		...
		self.boreYWdg = RO.Wdg.FloatEntry(
			master = sr.master,
			minValue = -60.0,
			maxValue = 60.0,
			defValue = DefBoresightYPos,
			helpText = "Boresight Y position",
			helpURL = HelpURL,
		)
		gr.gridWdg("Boresight Y", self.boreYWdg, "pixels")
		...
		gr.gridWdg("Centroid Radius", self.centroidRadWdg, "pixels", sticky="ew")
...
#		self.plotAxis.set_ylabel("FWHM (pixels)")
	...
	def run(self, sr):
		...
		self.tuiModel.tlSet.makeVisible(GuideTLName)
		...
		# shift boresight
		self.boreYPos = self.getEntryNum(self.boreYWdg, "Boresight Y")
		...
		for focInd in range(numFocPos):
			...
			sr.showMsg("Expose at focus position %d %sm" % \
		...
		if movebest:
			...
			self.logFocusMeas("Final", bestEstFocPos, finalFWHM)
	...
	def waitCentroid(self):
		...
		centroidCmdStr = "centroid time=%s bin=1 on=0,%s cradius=%s" % \
			(expTime, self.boreYPos, centroidRad)
		yield sr.waitCmd(
		   actor = GCamName,
	...
	def waitMoveBoresight(self):
		...
		# compute boresight position in arcsec
		desBorePix = [0, self.boreYPos]
		desBoreDeg = [desBorePix[ii] / self.instScale[ii] for ii in range(2)]
		
		# move boresight
		sr.showMsg("Moving the boresight")
		self.didMove = True
		cmdStr = "offset boresight %.7f, %.7f/pabs" % (desBoreDeg[0], desBoreDeg[1])
		yield sr.waitCmd(
			actor = "tcc",
			cmdStr = cmdStr,
		)
