...
#	print "begBoreXY=%r" % begBoreXY
	...
	driftSpeedAS = driftRangeAS / float(expTime)
	...
	while True:
		...
		if expNum % 2 == 0:
			...
			driftDir = -1
		startPosX = begBoreXY[0]
		startPosY = begBoreXY[1] - (driftRange / 2.0 * driftDir)
	
		# go to start position; use computed if first try
		# since we may have a significant distance to go
		if expNum == 0:
			sr.showMsg("Slew to starting position")
			optStr = "/computed"
		else:
			sr.showMsg("Move to starting position")
			optStr = ""

		tccCmdStr = "offset boresight %.7f, %.7f/pabs/vabs%s" % \
			(startPosX, startPosY, optStr)
