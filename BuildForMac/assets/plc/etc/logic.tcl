...
if 0 {					# Ctrl-button-1 doesn't work on os/x laptops
   set root_causes_button "<Control-Button-1>"
   set ignore_interlock_button "<Button-2>"
} else {
   set root_causes_button "<Button-2>"
   set ignore_interlock_button "<Control-Button-1>"
}
...
proc Logic {{drawname ""}} {
  ...
  global ignore_interlock_button root_causes_button
  ...
  if [info exists topdogs($drawname)] {
	...
	set drawname $label
     ...
     $canvas bind $glyph $root_causes_button "show_root_cause $drawname"
...
#     something like [AND bitA bitB] instead of simply "bitC"
	 ...
	 $canvas bind $glyph $root_causes_button "show_root_cause $val_name" 
	 ...
	 $canvas bind $glyph $ignore_interlock_button "toggle_mcpIgnoreValue $val"
	 $canvas bind $glyph $root_causes_button "show_root_cause $val"
  ...
  $canvas create text 80 $starty -fill $notecolor -tag deleteme \
   ...
   foreach tname $names {
	 ...
	 set fillcolor [Getfillcolor $tname]
      ...
      $canvas bind $glyph $root_causes_button "show_root_cause $tname"
...
proc show_root_cause {args} {
   global bigand_arrays bigor_arrays colors env root_causes ignore_interlock_button
   ...
   if {[info commands winfo] != "" && ![winfo exists .mcp_root_causes]} {
      ...
      pack [button .mcp_root_causes.bottom.close -text "close" -relief groove \
		-command "destroy .mcp_root_causes"] -side left
   ...
   if ![info exists root_causes] {
	 ...
	 label .mcp_root_causes.vars._$el.glyph \
	     -bitmap @$env(PLC_DIR)/etc/$bitmap \
	     -fg [Getfillcolor $el] -bd 0
	 bind .mcp_root_causes.vars._$el.glyph <Button-1> "Logic_info2 $el"
	 bind .mcp_root_causes.vars._$el.glyph $ignore_interlock_button \
...
proc update_bicolor {win type} {
   ...
   if {$val0} {
	 ...
	 set text  [lindex $texts 0]
      ...
      bind $win.$int.button <Button-1> "depress $win.$int.button; Logic_info2 [lindex $ints 0]" 
      bind $win.$int.button <Button-3> "depress $win.$int.button; Logic_info2 [lindex $ints 1]" 
   } else {
      if !$val1 {		# shouldn't both be on
	 ...
	 set text  [lindex $texts 1]
      ...
      bind $win.$int.button <Button-1> "depress $win.$int.button; Logic_info2 [lindex $ints 1]" 
      bind $win.$int.button <Button-3> "depress $win.$int.button; Logic_info2 [lindex $ints 0]" 
   }
...
proc graphicEngineMcp {} {
   ...
   if {[info commands winfo] != "" &&
     ...
     #   -text "Last Update: [utclock [getclock]]"
      }
...
proc label_as_button {args} {
   set window [lindex $args 0]

   set i 1
   while {$i < [llength $args]} {
      if {[lindex $args $i] == "-command"} {
	 incr i
   ...
   uplevel 1 "label {$window} $nargs -padx 7 -pady 5 -relief raised"
   if [info exists cmd] {
      uplevel 1 "bind {$window} <Button-1> {depress $window; $cmd}"
   }
}

proc key_interlocks {{toplevel ""} {geom ""}} {
   global canvas colors nestcount mcpData neutral_color mcp_main
   global root_causes_button ignore_interlock_button
   ...
   #
   global keydisplays root_causes_button
   ...
   foreach name [lsort [array names keydisplays]] {
	 ...
	 continue
      ...
      label_as_button $mcp_main.permits.$name -text $name -background $colors(led_ok)

      bind $mcp_main.permits.$name $root_causes_button "depress $mcp_main.permits.$name; show_root_cause $name"
      bind $mcp_main.permits.$name <Button-1> "depress $mcp_main.permits.$name; Logic_nameshow2 $name 1"
   ...
   foreach one $bicolor_status_list {
      ...
      label_as_button $mcp_main.hwstat.left.$thisn.button -background $neutral_color
   ...
   foreach name [lsort [array names instrument_changes]] {
	 ...
	 continue
      ...
      label_as_button $instrument_changes(.window).$name -text $name \
	  -background $colors(led_ok)
      bind $instrument_changes(.window).$name $root_causes_button "show_root_cause $name"
   ...
   foreach one $big_button_status_args {
	 ...
	 label_as_button $mcp_main.hwstat.right.$thisn.$caption -text $caption \
	     -background $colors(led_ok) \
	     -command "Logic_info2 $thisvar_here {} 1 {} 1"
      bind $mcp_main.hwstat.right.$thisn.$caption $ignore_interlock_button \
...
proc show_mcp_interlocks {{geom ""} {restart 0}} {
   ...
   global sort_interlocks_by_class show_interlocks_regexp ignore_interlock_button
   ...
   foreach int $fields {
	 ...
	 if [from_udp_packet $int] {
      ...
      label $canvas.$int.symb -bitmap @$env(PLC_DIR)/etc/$bitmap \
	 ...
	 bind $canvas.$int.$type $ignore_interlock_button \
...
proc make_ignored_interlocks {{geom ""}} {
   global colors mcpIgnoreValue ignore_interlock_button
   ...
   if [winfo exists .mcp_ignored] {
	 ...
	 wm geometry .mcp_ignored $geom
      ...
      pack [label .mcp_ignored.text \
		-text "\
 $ignore_interlock_button on an interlock bit
